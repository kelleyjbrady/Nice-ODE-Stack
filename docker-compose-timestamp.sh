...
TENSORBOARD_IMAGE_BASE_NAME="tensorboard" # <--- ADDED
COMPOSE_GEMMA=0
...
export BUILD_TAG=${DEV_BUILD_TAG} 
...
# --- TensorBoard / XProf Image Build Process (ADDED) ---
echo ""
echo "Starting TENSORBOARD IMAGE build process..."
FULL_TENSORBOARD_IMAGE_TAG_WITH_REGISTRY="${REGISTRY_PATH}/${TENSORBOARD_IMAGE_BASE_NAME}:${DEV_BUILD_TAG}"

echo "Building TENSORBOARD image: ${FULL_TENSORBOARD_IMAGE_TAG_WITH_REGISTRY}"
# Assumes your Dockerfile is inside a folder named `tensorboard`
docker build --no-cache -t "${FULL_TENSORBOARD_IMAGE_TAG_WITH_REGISTRY}" ./tensorboard/

if [ "$PUSH_TO_REG" -eq 1 ]
then
    echo "Pushing TENSORBOARD image: ${FULL_TENSORBOARD_IMAGE_TAG_WITH_REGISTRY}"
    docker push "${FULL_TENSORBOARD_IMAGE_TAG_WITH_REGISTRY}"
    echo "--------------------------------------------------"
    echo " TENSORBOARD IMAGE Build Complete!"
    echo " Pushed: ${FULL_TENSORBOARD_IMAGE_TAG_WITH_REGISTRY}"
    echo "--------------------------------------------------"
else
    echo "Image push skipped because PUSH_TO_REG==0"
    echo "--------------------------------------------------"
...
if [ ! -d "${PK_ANALYSIS_REPO_PATH}" ]; then
    ...
    echo "TENSORBOARD_IMAGE_TAG=${FULL_TENSORBOARD_IMAGE_TAG_WITH_REGISTRY}"
    exit 1
fi
...
TENSORBOARD_IMAGE_TAG=${FULL_TENSORBOARD_IMAGE_TAG_WITH_REGISTRY} # <--- ADDED
...
if [ -f "${PK_ANALYSIS_ENV_FILE}" ]; then
    # ADDED -e '^TENSORBOARD_IMAGE_TAG=' to the grep command to prevent duplicates
    EXISTING_OTHER_VARS=$(grep -v -e '^DEV_IMAGE_TAG=' -e '^MLFLOW_IMAGE_TAG=' -e '^GEMMA_IMAGE_TAG=' -e '^TENSORBOARD_IMAGE_TAG=' -e '^HF_TOKEN=' -e '^POSTGRES_USER=' -e '^POSTGRES_PASSWORD=' -e '^POSTGRES_DB=' -e '^DB_HOST=' -e '^DB_PORT=' -e '^# This file is managed' "${PK_ANALYSIS_ENV_FILE}" || true)
